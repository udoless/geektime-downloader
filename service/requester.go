...
//获取文章信息
func (s *Service) requestArticleInfo(aid int) (io.ReadCloser, Error) {
	data := map[string]interface{}{
		"id": aid,
	}
	res, err := s.client.Req("POST", "https://time.geekbang.org/serv/v3/article/info", data, map[string]string{"Origin": "https://time.geekbang.org"})
	return handleHTTPResponse(res, err)
}
