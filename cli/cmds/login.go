...
func loginAction(c *cli.Context) error {
	...
	if LoginConfig.IsByPhoneAndPassword() {
		err = loginByPhoneAndPassword(LoginConfig.phone, LoginConfig.password)
		if err != nil {
			return err
		}
		return nil
	} else if gcid != "" && gcess != "" {
		geektime, err := config.Instance.SetUserByGcidAndGcess(gcid, gcess, serverID, LoginConfig.phone, LoginConfig.password)
		if err != nil {
			return err
		}
...
func loginByPhoneAndPassword(phone string, password string) error {
	gcid, gcess, serverID, err := application.Login(phone, password)
	if err != nil {
		return err
	}

	geektime, err := config.Instance.SetUserByGcidAndGcess(gcid, gcess, serverID, phone, password)
	if err != nil {
		return err
	}
